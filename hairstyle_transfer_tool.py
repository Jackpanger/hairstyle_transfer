...
class Tool:
    ...
    def __init__(self, opts=None, result_path='./data/results/', checkpoint_path='/content/drive/MyDrive/model/best_model.pt'):
        if opts is None:
            self.opts = Namespace()
        ...
        if os.path.exists(self.__prepare_filename(im1_path, None, alpha_blend, kwargs={'phase':'manip',  'dir':direction_name,  'strength':coeffs[0]})):
            ...
            if self.net is None:
                ...
                if alpha_blend:
                    ...
                    inner_face1, hair1 = utils.load_and_split_image(im1_path)
            im1_orig = inner_face1 + hair1
            with open('/content/drive/MyDrive/model/latent_directions_hair_space.pkl', 'rb') as (f):
